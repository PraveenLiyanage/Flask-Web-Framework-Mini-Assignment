...
# Search operation: Display search transaction form
...
def search_transactions():
    if request.method == 'POST':
        ...
        for transaction in transactions:
            ...
            if (not search_date or transaction_date == search_date) and \
               (min_amount <= transaction_amount <= max_amount):
...
# Total balance operation: Calculate and display total balance
@app.route("/balance")
def total_balance():
    # Calculate the total balance by summing the 'amount' values of all transactions
    total = sum(transaction['amount'] for transaction in transactions)
    
    # Render the 'transactions.html' template with both transactions and total balance
